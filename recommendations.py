...
def get_recommendations(client_recent_purchases, client_previous_month_data, client_current_month_data, df_recent_purchases, segmentation_df, client_id):
    recommendations = []

    # Comparer les dépenses entre les deux mois dynamiques
    previous_month_spending = client_previous_month_data['GMV'].sum()
    current_month_spending = client_current_month_data['GMV'].sum()

    if current_month_spending < previous_month_spending:
        recommendations.append({
            "Type": "Augmentation des efforts",
            "Recommandation": "Le client a baissé ses dépenses ce mois-ci par rapport au mois précédent. Augmentez les efforts de recommandation pour l'inciter à commander plus.",
            "Détails": ""
        })
    elif current_month_spending > previous_month_spending:
        recommendations.append({
            "Type": "Maintien des efforts",
            "Recommandation": "Le client a augmenté ses dépenses ce mois-ci par rapport au mois précédent. Maintenez ou améliorez les recommandations pour continuer cette tendance.",
            "Détails": ""
        })
    ...
    previous_month_categories = client_previous_month_data['Product Category'].nunique()
    current_month_categories = client_current_month_data['Product Category'].nunique()

    # Comparaison entre les deux mois dynamiques
    if current_month_categories < previous_month_categories:
        categories_not_bought_in_current_month = set(client_previous_month_data['Product Category'].unique()) - set(client_current_month_data['Product Category'].unique())
        recommendations.append({
            "Type": "Recommandation de catégories",
            "Recommandation": "Recommandez des achats dans les catégories non commandées ce mois-ci.",
            "Détails": f"Catégories non commandées ce mois-ci: {', '.join(categories_not_bought_in_current_month)}"
        })
