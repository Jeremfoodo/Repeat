...
def segmentation_page(df):
    ...
    # Ajouter des boutons de lien vers la page client_info pour chaque client dans les dataframes
    def add_client_info_button(df, col_name='Client Info'):
        df[col_name] = df['Restaurant ID'].apply(lambda id: f'<a href="?page=client_info&client_id={id}" target="_self">Voir Info</a>')
        return df
    ...
    inactive_clients = add_client_info_button(inactive_clients)
    st.write(inactive_clients[['Restaurant ID', 'Restaurant', 'Segment', 'Spending Level', 'Total', 'Dernière commande', 'Client Info']], unsafe_allow_html=True)
    st.download_button(
    ...
    downgraded_clients = add_client_info_button(downgraded_clients)
    st.write(downgraded_clients[['Restaurant ID', 'Restaurant_Juin', 'Spending Level_Juin', 'Total_Juin', 'Spending Level_Juillet', 'Total_Juillet', 'Dernière commande', 'Client Info']], unsafe_allow_html=True)
    st.download_button(
    ...
    same_tier_less_spending_clients = add_client_info_button(same_tier_less_spending_clients)
    st.write(same_tier_less_spending_clients[['Restaurant ID', 'Restaurant_Juin', 'Spending Level_Juin', 'Total_Juin', 'Total_Juillet', 'Dernière commande', 'Client Info']], unsafe_allow_html=True)
    st.download_button(
    ...
    st.markdown("<small>Ces clients ont augmenté leurs dépenses par rapport à juin, félicitations !</small>", unsafe_allow_html=True)
    increased_spending_clients = add_client_info_button(increased_spending_clients)
    st.write(increased_spending_clients[['Restaurant ID', 'Restaurant_Juin', 'Spending Level_Juin', 'Total_Juin', 'Total_Juillet', 'Dernière commande', 'Client Info']], unsafe_allow_html=True)
    st.download_button(
